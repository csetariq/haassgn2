        ...
        jsonData.read('users', userFileName, function(err, userData) {
            if (!err && userData) {
                ...
                if (hashedPassword == userData.password) {
                    var tokenData = {
                        id: helpers.generateRandomString(config.tokenLength),
                        email: email,
                        expires: Date.now() + config.tokenValidityInMillis
                    };
...
handlers.put = function(data, callback) {
    ...
    if (id && extend) {
        jsonData.read('tokens', id, function(err, tokenData) {
            if (!err && tokenData) {
                tokenData.expires = Date.now() + config.tokenValidityInMillis;
...
handlers.getUser = function(id, callback) {
    jsonData.read('tokens', id, function(err, tokenData) {
        if (!err) {
