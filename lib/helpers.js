...
lib.generateRandomString = function(stringLength) {
...
lib.hash = function(stringData) {
    if (typeof(stringData) == 'string' && stringData.length > 0) {
        var hash = crypto.createHmac('sha256', 'hashingSecret').update(stringData).digest('hex');
        return hash;
    } else {
        return false;
    }
